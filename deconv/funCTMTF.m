function [fx_out, MTF_out, V50, V10, theta_out] = funCTMTF(I,pixelL,pname)
...
for theta = (theta - Htheta) : 0.01 : (theta + Htheta)
    ...
    dx = xp(2:end);
    if plotFlag
        figure(1);subplot(2,2,2);plot(dx,dR);title(theta);
    end
    ...
    if plotFlag
        figure(1);subplot(2,2,2);hold on; plot(dx,dR)
